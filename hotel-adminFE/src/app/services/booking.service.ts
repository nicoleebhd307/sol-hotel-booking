...
// Fallback mock contains only active/paid bookings (confirmed, checked_in, checked_out)
...
export class BookingService {
  ...
  getBookings(): Observable<Booking[]> {
    // Only fetch active/paid bookings (confirmed, checked_in, checked_out)
    ...
    return this.http.get<unknown>(this.apiUrl).pipe(
      ...
      map((bookings) => bookings.filter(b => activeStatuses.has(b.status ?? ''))),
