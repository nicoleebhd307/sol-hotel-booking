      ...
      .subscribe({
        next: (response) => {
          this.ngZone.run(() => {
            ...
            const options = new Set(this.nationalityOptions.filter((item) => item !== 'all'));
            this.customers.forEach((customer) => options.add(customer.nationality));
            this.nationalityOptions = ['all', ...Array.from(options).sort()];
